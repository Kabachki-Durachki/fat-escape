...
class Player:
    ...
    def move(self, map):
        ...
        # Проверяем, является ли клетка непроходимой (2.png, 10.png или 11.png)
        if (
                map[tile_y][tile_x].content == r'2.png'
                or map[tile_y][tile_x].content == r'10.png'
                or map[tile_y][tile_x].content == r'11.png'  # Добавлено
        ):
            self.position = self.prev_position
            self.rect.center = self.position

            # Попытка "скольжения" вдоль стен (опционально)
            ...
            if (
                    map[tile_y_new][tile_x_new].content != r'2.png'
                    and map[tile_y_new][tile_x_new].content != r'10.png'
                    and map[tile_y_new][tile_x_new].content != r'11.png'  # Добавлено
            ):
...
class Enemy:
    def __init__(self, x, y):
        ...
        self.is_dead = False
        self.target_cell = (14.5, 3)  # Координаты целевой клетки в тайлах
        self.health_loss_rate = 1  # Потеря здоровья за шаг
        self.last_move_time = 0

    def move_to_target(self, map):
        target_x = self.target_cell[0] * TILE_SIZE + TILE_SIZE / 2
        target_y = self.target_cell[1] * TILE_SIZE + TILE_SIZE / 2
    ...
    def move_towards_player(self, player, map):
        ...
        if current_time - self.last_move_time > 100:
            ...
            if self.health <= 0:
                self.image = pygame.image.load('femka_pohudevshaya.png').convert_alpha()
                self.original_image = self.image
                self.is_dead = True
                return

        # Получаем позицию из объекта игрока
    ...
    def can_see_player(self, player_position, map):
        ...
        for i in range(steps + 1):
            x = self.position[0] + step_x * i
            y = self.position[1] + step_y * i
    ...
    def draw(self, screen):
        ...
        if not self.is_dead and self.health<50:
            pygame.draw.rect(screen, RED, (self.rect.left, self.rect.top - 10, 50, 5))
            pygame.draw.rect(screen, GREEN, (self.rect.left, self.rect.top - 10, self.health, 5))
...
class Tile():
    def __init__(self, tile_type: int):
        ...
        if tile_type == 11:
            self.image = pygame.transform.scale(self.image, [1920, 262])
...
class Map():
    ...
    def generate(self, x = 30, y = 17):
        ...
        self.spawn_x = random.randint(1, x - 2)
        self.spawn_y = random.randint(4, y - 2)
        ...
        self.map[0][0] = Tile(2)
        for i in range(len(self.map[0])):
            self.map[0][i] = Tile(10)
            self.map[1][i] = Tile(10)
            self.map[2][i] = Tile(10)
            self.map[3][i] = Tile(10)

        self.map[0][0] = Tile(11)
...
while run:
    if show_menu:
        ...
        display.fill(WHITE)
        text = font.render("Fat escape", True, BLACK)
        ...
        for enemy in enemies[:]:
            if enemy.is_dead:
                reached_target = enemy.move_to_target(karta.map)
                enemy.draw(display)  # Отрисовываем даже мертвых
                if reached_target:
                    enemies.remove(enemy)
            else:
                if enemy.can_see_player(play.position, karta.map):
                    enemy.move_towards_player(play, karta.map)
                    # Проверка дистанции для атаки
                    distance = math.hypot(
                        play.position[0] - enemy.position[0],
                        play.position[1] - enemy.position[1]
                    )
                    if distance < 50:
